...
class QuestionClassifier:  # 问题分类
    def __init__(self):
        ...
        # 特征词路径
        self.disease_path = os.path.join(cur_dir, 'dict/disease.txt')  # 疾病词典路径
        ...
        self.department_wds= [i.strip() for i in open(self.department_path,encoding="utf-8") if i.strip()]  # 剥离字符串中的空白符
        ...
        self.region_words = set(self.department_wds + self.disease_wds + self.check_wds + self.drug_wds + self.food_wds + self.producer_wds + self.symptom_wds)  # 用户输入的语句在region_words中进行匹配
